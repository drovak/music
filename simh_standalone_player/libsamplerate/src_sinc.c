...
{	double		fraction, left, right, icoeff ;
	...
	int count = 0;
#endif
	int first_filter_index = filter_index;

	left = 0.0 ;
	for (int i = 0; i < (first_filter_index / increment) + 1; i++)
	{	
	...
	if (while_count != count)
	{
		printf("(left) while: %d\t for: %d\n", while_count, count);
		printf("ffi: %d\ti: %d\tdiv: %d\n", first_filter_index, increment, (first_filter_index / increment) + 1);
	}
#endif
